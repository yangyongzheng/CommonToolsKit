...
/**
 ...
 @return 比较结果
 */
CMLKIT_STATIC_INLINE NSComparisonResult CFLCompareVersion(NSString *v1, NSString *v2) {
    // 以小数点结尾的补'0'再比较
    if ([v1 hasSuffix:@"."]) {
...
CMLKIT_STATIC_INLINE BOOL CFLSystemVersionEqualTo(NSString *version) {
    return CFLCompareVersion(UIDevice.currentDevice.systemVersion, version) == NSOrderedSame;
}

CMLKIT_STATIC_INLINE BOOL CFLSystemVersionGreaterThan(NSString *version) {
    return CFLCompareVersion(UIDevice.currentDevice.systemVersion, version) == NSOrderedDescending;
}

CMLKIT_STATIC_INLINE BOOL CFLSystemVersionGreaterThanOrEqualTo(NSString *version) {
    return CFLCompareVersion(UIDevice.currentDevice.systemVersion, version) != NSOrderedAscending;
}

CMLKIT_STATIC_INLINE BOOL CFLSystemVersionLessThan(NSString *version) {
    return CFLCompareVersion(UIDevice.currentDevice.systemVersion, version) == NSOrderedAscending;
}

CMLKIT_STATIC_INLINE BOOL CFLSystemVersionLessThanOrEqualTo(NSString *version) {
    return CFLCompareVersion(UIDevice.currentDevice.systemVersion, version) != NSOrderedDescending;
}


#pragma mark - 基本数据类型判空函数
CMLKIT_STATIC_INLINE BOOL CFLAssertStringNotEmpty(id str) {
    return str && [str isKindOfClass:[NSString class]] && ((NSString *)str).length > 0;
}

CMLKIT_STATIC_INLINE BOOL CFLAssertStringIsEmpty(id str) {
    return !CFLAssertStringNotEmpty(str);
}

CMLKIT_STATIC_INLINE BOOL CFLAssertArrayNotEmpty(id array) {
    return array && [array isKindOfClass:[NSArray class]] && ((NSArray *)array).count > 0;
}

CMLKIT_STATIC_INLINE BOOL CFLAssertArrayIsEmpty(id array) {
    return !CFLAssertArrayNotEmpty(array);
}

CMLKIT_STATIC_INLINE BOOL CFLAssertMutableArrayNotEmpty(id mutableArray) {
    return mutableArray && [mutableArray isKindOfClass:[NSMutableArray class]] && ((NSMutableArray *)mutableArray).count > 0;
}

CMLKIT_STATIC_INLINE BOOL CFLAssertMutableArrayIsEmpty(id mutableArray) {
    return !CFLAssertMutableArrayNotEmpty(mutableArray);
}

CMLKIT_STATIC_INLINE BOOL CFLAssertDictionaryNotEmpty(id dictionary) {
    return dictionary && [dictionary isKindOfClass:[NSDictionary class]] && ((NSDictionary *)dictionary).count > 0;
}

CMLKIT_STATIC_INLINE BOOL CFLAssertDictionaryIsEmpty(id dictionary) {
    return !CFLAssertDictionaryNotEmpty(dictionary);
}

CMLKIT_STATIC_INLINE BOOL CFLAssertMutableDictionaryNotEmpty(id mutableDictionary) {
...
CMLKIT_STATIC_INLINE BOOL CFLAssertMutableDictionaryIsEmpty(id mutableDictionary) {
...
#pragma mark - 安全主线程回调
// 1.同步回调主线程
CMLKIT_STATIC_INLINE void CFLSafeSyncMainQueueHandler(void(^handler)(void)) {
    if (handler) {
...
// 2.异步回调主线程
CMLKIT_STATIC_INLINE void CFLSafeAsyncMainQueueHandler(void(^handler)(void)) {
    if (handler) {
        if ([NSThread isMainThread]) {
            handler();
        } else {
