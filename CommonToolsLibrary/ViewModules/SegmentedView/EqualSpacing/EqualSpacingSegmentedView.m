...
#import "ESSVNormalCollectionViewCell.h"
#import "ESSVBadgeTextCollectionViewCell.h"
#import "ESSVBadgeImageCollectionViewCell.h"


@interface EqualSpacingSegmentedView ()<UICollectionViewDelegateFlowLayout, UICollectionViewDataSource>
@property (nonatomic, strong) UICollectionView *collectionView;
@property (nonatomic) NSUInteger selectedIndex;
@end

@implementation EqualSpacingSegmentedView
...
- (instancetype)initWithFrame:(CGRect)frame
                ...
                configuration:(SegmentedViewConfiguration *)configuration
                     delegate:(id<EqualSpacingSegmentedViewDelegate>)delegate {
...
- (void)setSelectItemAtIndex:(NSUInteger)index
    ...
    if (index != self.selectedIndex && index < self.itemsArray.count) {
        ...
        if (invokeDelegateMethod &&
            [self.delegate respondsToSelector:@selector(equalSpacingSegmentedView:didSelectItem:atIndex:)]) {
            [self.delegate equalSpacingSegmentedView:self
...
- (void)initializeConfiguration {
    ...
    [self.collectionView registerNib:[UINib nibWithNibName:NSStringFromClass([ESSVNormalCollectionViewCell class])
                                                    bundle:[NSBundle bundleForClass:[EqualSpacingSegmentedView class]]]
          forCellWithReuseIdentifier:@"normal cell"];
    [self.collectionView registerNib:[UINib nibWithNibName:NSStringFromClass([ESSVBadgeTextCollectionViewCell class])
                                                    bundle:[NSBundle bundleForClass:[EqualSpacingSegmentedView class]]]
          forCellWithReuseIdentifier:@"badge text cell"];
    [self.collectionView registerNib:[UINib nibWithNibName:NSStringFromClass([ESSVBadgeImageCollectionViewCell class])
                                                    bundle:[NSBundle bundleForClass:[EqualSpacingSegmentedView class]]]
          forCellWithReuseIdentifier:@"badge image cell"];
...
- (void)updateIndicatorViewAnimated:(BOOL)animated {
    if (self.itemsArray.count > 0 && self.selectedIndex < self.itemsArray.count) {
        ...
        [self calculateWidthWithItem:item atIndex:self.selectedIndex result:^(CGFloat titleWidth, CGFloat contentWidth) {
            ...
            if (width < 0) {width = 0;}
            CGFloat x = CGRectGetMinX(attributes.frame) + strongSelf.configuration.indicatorOffset;
...
- (void)calculateWidthWithItem:(SegmentedViewItem *)item atIndex:(NSInteger)index result:(void(^)(CGFloat titleWidth, CGFloat contentWidth))result {
    UIFont *titleFont = index == self.selectedIndex ? self.configuration.selectedTitleFont : self.configuration.unselectedTitleFont;
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    if (self.itemsArray.count > 0) {
        ...
        [self calculateWidthWithItem:item
                             atIndex:indexPath.item
                              result:^(CGFloat titleWidth, CGFloat contentWidth) {
                                  itemWidth = contentWidth;
                              }];
        return CGSizeMake(itemWidth, floor(self.bounds.size.height));
    } else {
        return CGSizeZero;
    }
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    SegmentedViewItem *item = self.itemsArray[indexPath.item];
    if (item.badgeImage.imageName.length > 0 && !CGSizeEqualToSize(item.badgeImage.imageSize, CGSizeZero)) {
        ESSVBadgeImageCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"badge image cell" forIndexPath:indexPath];
        ...
        cell.badgeImageView.backgroundColor = [UIColor clearColor];
        ...
        cell.badgeImageViewBottomConstraint.constant = item.badgeOffset.vertical;
        [cell layoutIfNeeded];
        return cell;
    } else if (item.badgeText.badgeValue.length > 0) {
        ESSVBadgeTextCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"badge text cell" forIndexPath:indexPath];
        cell.titleLabel.text = item.title;
        cell.titleLabel.font = self.selectedIndex == indexPath.item ? self.configuration.selectedTitleFont : self.configuration.unselectedTitleFont;
        cell.titleLabel.textColor = self.selectedIndex == indexPath.item ? self.configuration.selectedTitleColor : self.configuration.unselectedTitleColor;
        ...
        ESSVNormalCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"normal cell" forIndexPath:indexPath];
        cell.titleLabel.text = item.title;
        cell.titleLabel.font = self.selectedIndex == indexPath.item ? self.configuration.selectedTitleFont : self.configuration.unselectedTitleFont;
        cell.titleLabel.textColor = self.selectedIndex == indexPath.item ? self.configuration.selectedTitleColor : self.configuration.unselectedTitleColor;
        return cell;
